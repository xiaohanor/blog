...
宏：DECLARE\_DELEGATE、DECLARE\_DELEGATE\_OneParam
...
您可以在引擎安装文件夹中找到这些代码模块的示例（例如。Epic Games\\UE\_5.0\\Engine\\Source\\Runtime\\AIModule），其中每个模块都有自己的 \[YourModuleName\].build.cs 文件来配置自身及其依赖项。
