...
date: 2025-03-18
lastmod: 2025-03-18
tags: ['UE', 'cpp', '教程', '持久化数据', '网络']
draft: false
summary: ' Unreal Engine 跨关卡和断开连接时如何持久化数据'
images:
authors:
  - default
layout: PostLayout
...
> 原文：[Unreal Engine Persistent Data Compendium - WizardCell](https://wizardcell.com/unreal/persistent-data/) > **提示**：当前版本的 _无缝转移(Seamless Travel)_ 与 _世界分区_ 功能存在冲突，请关闭关卡的世界分区
...
- **TRAVEL_Relative**（相同服务器，并保留最后一个 [选项字符串](https://wizardcell.com/unreal/persistent-data/#7-game-options-string)）：当前 [URL](https://wizardcell.com/unreal/persistent-data/#url-structure) 相对于上一个 URL，因此我们不会与服务器断开连接，这使得它非常适合**无缝**转移。因此，当客户端进行**无缝**转移时，这种转移类型是必需的。最后一个选项字符串会带到新关卡。
- **TRAVEL_Partial**（服务器重置，但保留最后一个选项字符串）：当前 URL 部分匹配上一个 URL，因此我们与服务器断开连接，这对应于**非无缝**转移。最后一个选项字符串会被带到新地图。
- **TRAVEL_Absolute**（服务器重置，并忽略最后一个选项字符串）：当前 URL 是绝对的，这意味着最后一个 URL（包括最后一个选项字符串）被清除，因此我们与服务器断开连接，这对应于**非无缝**转移。
...
## [GameFramework](https://docs.unrealengine.com/4.27/en-US/InteractiveExperiences/Framework/) 对象
